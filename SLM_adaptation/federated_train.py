...
def run_federated(method, global_model, tokenizer, full_df, availability_df, cfg, round_end_callback=None):
    ...
    for r in range(cfg.NUM_ROUNDS):
        ...
        if method == "random":
            sel = random_scheduler(avail, cfg.CLIENTS_PER_ROUND, rng)
        elif method == "aws":
            sel = aws_scheduler(round_df, availability_df, persona_stats, last_seen, cfg.CLIENTS_PER_ROUND, rng)
        elif method == "oracle":
            sel = always_available_oracle(all_clients, cfg.CLIENTS_PER_ROUND, rng)
        elif method == "no_availability":
            sel = no_availability_awareness(all_clients, cfg.CLIENTS_PER_ROUND, rng)
        else:
            sel = []
        ...
        if round_end_callback is not None:
            round_end_callback(global_model, tokenizer, r, row)
